...
// Solution code here...
  ...
  const arrLengths = arr.map(str => str.length)
  return arrLengths.reduce((acc, curr, index) => {
    if (arrLengths[index] > arrLengths[acc]){
      acc = index;
    }
    return acc;
  }, 0);
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  //   return arr.reduce((acc, curr) => { figure out regex
