...
const sortByChildren = (charArray) => {
  ...
  charArray.sort((a,b)=> {
    if( a.children.length === b.children.length){
      let nameA = a.house.toUpperCase();
      let nameB = b.house.toUpperCase();
      if (nameA < nameB){
        return -1;
      } if (nameA > nameB){
        return 1;
  ...
  return charArray
  // look through every character to the children key
  // for(let i in charArray){
  //   console.log(charArray[i].sort(children));
  //   if(charArray[i].children === charArray[i].children){
  //     charArray[i].sort(house);
  //   }
  // }
  // use .sort() on children key

  // check if number if the number of children is the same if so sort alphabetically by house name
